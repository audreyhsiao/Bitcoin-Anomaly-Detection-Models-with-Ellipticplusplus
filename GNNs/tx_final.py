...
from torch_geometric.nn import GCNConv,FiLMConv,GATConv
...
tx = pd.concat([tx.iloc[:, :166], tx['class']], axis=1)
...
class GAT_Net(torch.nn.Module):
  def __init__(self, features, hidden, classes, heads):
    ...
    self.gat1 = GATConv(features, hidden, heads=heads,dropout=0.3)
    self.gat2 = GATConv(hidden * heads, classes,dropout=0.6)
  def forward(self, data):
      x, edge_index = data.x, data.edge_index
    ...
    for hidden in [128]:
        for heads in [8,10]:
                ...
                for epoch in range(1, num_epochs + 1):
                    train_loss, train_acc = train(model, criterion=criterion, train_loader=train_loader)

                    if epoch % print_every == 0:
                        test_loss, test_acc = test(model, criterion=criterion, test_loader=test_loader)
                ...
                auc_score = drawROC(y_true,y_pred)
                print('hidden layer:%d, heads:%d, weight:%.2f '%(hidden,heads,weight))
                print("Final Metrics:")
