...
df_edges = pd.read_csv('trainData/actor.csv')
...
edge_index,_ = load_edge_csv(df_actor, 'input_address',  df_actor_mapping , 'output_address',  df_actor_mapping ,
                  encoders=None)
...
model = GNN(num_features=df_actor.shape[1]-1, num_classes=3).to(device)
data = data.to(device)
...
torch.save(model, './model/df_actor.pth')
